...
# attaching user photos
  ...
  Spot.create(
    ...
    host_id: User.all[(1..34).to_a.sample].id,
    num_guests: 3,
    num_bedrooms: 1,
    num_beds: 1,
    num_baths: 1,
  ...
  Spot.all.each_with_index do |spot,idx|
    (1..3).each do |pic_idx|
      ...
      if idx < 9
        ...
        spot.photos.attach(io: file, filename: "0#{pic_idx}-SF.jpg")
      else
        file = EzDownload.open("https://s3.console.aws.amazon.com/s3/object/waterbnb-seed/spots/SF/#{idx+1}-SF/#{idx+1}-0#{pic_idx}-SF.jpg?region=us-east-2&tab=overview")
        spot.photos.attach(io: file, filename: "#{pic_idx}-SF.jpg")
      end
      spot.save! 
    end
  end
